...
let Log = {
  updateBuild(files, updateCountShown) {
    ...
    var updatedAt = new Date(),
      updatedAtDisplay = `[${updatedAt.getHours()}:${updatedAt.getMinutes()}:${(
    ...
    if (
      ...
      let getMessage = osNotice => {
        ...
        if (!osNotice) {
          ...
          return `v${printedVersion} (${seconds}s)`;
        }
      };
...
function scripts(p) {
  ...
  // wrap in watchify
  if (watch) {
    b = watchify(b, WATCHIFY_CONFIG);
  }

  b.on("error", err => {
    if (watch) {
      Log.errorWhileRebuilding(err);
    } else {
      Log.errorWhileRebuilding(err);
    }
...
gulp.task("minify-images", function() {
  return gulp
    .src(Paths.DEST_DEV + "/**/*.+(png|jpg|gif|svg)")
    .pipe(imagemin())
    .pipe(gulp.dest(Paths.DEST_PROD));
});

gulp.task("copy-extra-to-build", function() {
  return gulp.src(Paths.DIST_DEV_FILES).pipe(gulp.dest(Paths.DEST_PROD));
});
...
gulp.task("check-for-version-bump", function() {
  ...
  let versionShouldBeBumped =
    versionFileContent.indexOf(latestVersionBuilt) != -1 &&
    FlagValues.version_bump;
  if (versionShouldBeBumped) {
    return gulp
      .src([Paths.VERSION_CONFIG_FILE])
      .pipe(
        replace(
          /VERSION[\s]*:[\s]*'([\d]+)[\.]([\d]+)[\.]([\d]+)'/gi,
...
gulp.task("export-standalone-build", () => {
  if (FlagValues.export_standalone) {
    ...
    return gulp.src(Paths.BUILD_PACKAGE_SOURCES).pipe(gulp.dest(buildDir));
  }
});
